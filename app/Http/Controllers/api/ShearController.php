...
class ShearController extends Controller
{
    ...
    // Global Variables
    private $M = 0; // количество ребер (number of edges)
    private $K = 0; // количество контуров (number of contours)
    private $N = 0; // количество вершин (number of vertices)

    private $Bn = $this->new_array(Mat); // Ордината вершины
    private $Yn = $this->new_array(Mat); // Абсцисса вершины
    private $Tm = $this->new_array(Mat); // Приведенная толщина ребра
    private $ks = $this->new_array(Mat); // Коэффициент приведения толщины

    private $C = null; // Матрица индексов

    private $Bm = $this->new_array(Mat); //Инициализационная точка ординаты ребра
    private $Ym = $this->new_array(Mat); //Инициализационная точка абсциссы ребра
    private $Q1 = $this->new_array(Mat); //Ориентация ребра
    private $QL = $this->new_array(Mat); //Длина ребра

    // In Pascal, C is declared as pointer to MatrMas; here we simulate it as a 2D array.
    private $X1 = null; // will be allocated as an array of double (1D array)
    private $X2 = null; // same as above

    private $B0 = 0.0; //отстояние н.о. Y  от о.с. , СМ
    private $Y0 = 0.0; //отстояние н.о. Z  от о.с., СМ
    private $F = 0.0; //Площадь поперечного сечения, см2
    private $Iy0 = 0.0; //момент инерции относительно оси Y, СМ4
    private $Msw = 0.0; //изгибающий момент, кгс*см

    private $Nsw = 0.0; //перерезывающую силу, кгс

    private $Iz0 = 0.0; //момент инерции относительно оси Z, СМ4
    private $OM = 0.0; //Приведенная площадь по сдвигу, см2
    ...
    private function Gauss($P, $X, $Y, &$Z)
    {
        ...
        $Z[$P] = $Y[$P] / $X[$P][$P]; // Обратная подстановка
        for ($L = 2; $L <= $P; $L++) {
            $I = $P + 1 - $L;
            $F = 0;
            $MM = $L - 1;
            for ($J = 1; $J <= $MM; $J++) {
                $F = $F + $Z[$P + 1 - $J] * $X[$I][$P + 1 - $J];
            }
            $Z[$I] = ($Y[$I] - $F) / $X[$I][$I];
        }
    }
    ...
    private function Cont(&$B0, &$Y0, &$F, &$Iy0, &$Iz0, &$OM, &$X1, &$X2)
    {
        ...
        $G = $this->new_matrix(Mat, Mat, 0.0);
        $As = $this->new_matrix(Mat, Mat, 0.0);
        $Au = $this->new_matrix(Mat, Mat, 0.0);
        $AA = $this->new_matrix(Mat, Mat, 0.0);
        $Nt = $this->new_matrix(Mat, Mat, 0.0);

        // One dimensional arrays of length Mat (simulate MD)
        $X = $this->new_array(Mat, 0.0);
        $FI = $this->new_array(Mat, 0.0);
        $S = $this->new_array(Mat, 0.0);
        $Um = $this->new_array(Mat, 0.0);
        $T0m = $this->new_array(Mat, 0.0);
        $Bs = $this->new_array(Mat, 0.0);
        $L = $this->new_array(Mat, 0.0);
        $BB = $this->new_array(Mat, 0.0);
        $Bu = $this->new_array(Mat, 0.0);
        $Hm = $this->new_array(Mat, 0.0);
        $A2k = $this->new_array(Mat, 0.0);
        $Fm = $this->new_array(Mat, 0.0);
        $Sym = $this->new_array(Mat, 0.0);
        $Szm = $this->new_array(Mat, 0.0);
        $Iym = $this->new_array(Mat, 0.0);
        $Izm = $this->new_array(Mat, 0.0);
        $Iyzm = $this->new_array(Mat, 0.0);
        $Psim = $this->new_array(Mat, 0.0);
        $Un = $this->new_array(Mat, 0.0);
        $DU = $this->new_array(Mat, 0.0);
